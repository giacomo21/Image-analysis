...
import sys
sys.path.append('/media/hdd2/ITK/wrapITK/lib/InsightToolkit/WrapITK/Configuration/Languages/SwigInterface/pygccxml-1.0.0/')
...
def generate_constructor( typedef, constructor, indent, w ):  
  ...
  for arg in constructor.arguments:
    s = "%s %s" % (get_alias(arg.type.decl_string, w), arg.name)
...
def generate_method( typedef, method, indent, w ):
  ...
  print >> outputFile, "  "*indent, "    %s%s %s(%s)%s;" % (static, get_alias(method.return_type.decl_string, w), method.name, ", ".join( args), const )
