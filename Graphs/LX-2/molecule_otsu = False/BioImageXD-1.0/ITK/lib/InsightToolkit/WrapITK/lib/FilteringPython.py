...
if version_info >= (3,0,0):
    new_instancemethod = lambda func, inst, cls: _FilteringPython.SWIG_PyInstanceMethod_New(func)
else:
...
if version_info >= (2,6,0):
    def swig_import_helper():
        ...
        fp = None
        try:
            fp, pathname, description = imp.find_module('_FilteringPython', [dirname(__file__)])
        except ImportError:
            import _FilteringPython
            return _FilteringPython
        if fp is not None:
            try:
                _mod = imp.load_module('_FilteringPython', fp, pathname, description)
            finally:
                fp.close()
            return _mod
    _FilteringPython = swig_import_helper()
