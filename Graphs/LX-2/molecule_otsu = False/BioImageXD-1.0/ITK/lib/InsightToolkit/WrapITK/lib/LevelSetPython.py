...
if version_info >= (3,0,0):
    new_instancemethod = lambda func, inst, cls: _LevelSetPython.SWIG_PyInstanceMethod_New(func)
else:
...
if version_info >= (2,6,0):
    def swig_import_helper():
        ...
        fp = None
        try:
            fp, pathname, description = imp.find_module('_LevelSetPython', [dirname(__file__)])
        except ImportError:
            import _LevelSetPython
            return _LevelSetPython
        if fp is not None:
            try:
                _mod = imp.load_module('_LevelSetPython', fp, pathname, description)
            finally:
                fp.close()
            return _mod
    _LevelSetPython = swig_import_helper()
