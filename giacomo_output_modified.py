...
# PAIRWISE INTER CONDITION - masks - histo + boxplot - single slices
	for i in range(0,len(data)-1):
		for j in range(i+1,len(data)):
			temp = select_arrays([data[i], data[j]], merged = False, conditions_name = ?, slices_name = ?, channels_name = ['nuclei', 'all'], what = [0])
...
# INTER CONDITION - masks - histo + boxplot - merged slices
	temp = select_arrays(data, merged = True, conditions_name = ?, slices_name = ?, channels_name = ?,  what = [0])
...
# SINGLE CONDITION - mask vs all - histo + boxplot - single slices
	for i in range(0,len(data)):
		temp = select_arrays([data[i]], merged = False, conditions_name = ?, slices_name = ?, channels_name = ['nuclei', 'all'], what = [0,1])
		histogram(temp[0], log=True, labels = temp[1], histtype='step', bins=128, color = ['green', 'red']*(len(temp)/2), outfile = out_folder + '/' + conditions_labels[i] + '_histogram.png')
...
# SINGLE CONDITION - mask vs all - histo + boxplot - merged slices
	for i in range(0,len(data)):
		temp = select_arrays([data[i]], merged = True, conditions_name = ?, slices_name = ?, channels_name = ['nuclei', 'all'], what = [0,1])
		histogram(temp[0], log=True, labels = temp[1], histtype='step', bins=128, color = ['green', 'red']*(len(temp)/2), outfile = out_folder + '/' + conditions_labels[i] + '_merged_histogram.png')
